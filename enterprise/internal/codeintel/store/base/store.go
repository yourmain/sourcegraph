...
// the correct behavior of the underlying base. Note that `With` and `Transact` must
// be re-defined in the outer layer in order to create a useful return value. Failure
// to re-define these methods will result in `With` and `Transact` methods that return
...
//     }
//
//     func (s *SprocketStore) With(other base.ShareableStore) *SprocketStore {
